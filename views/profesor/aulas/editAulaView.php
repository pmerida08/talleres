                ...
                <div class="divFormAddEdit">
                    ...
                    <form action="/admin/aulas/edit/<?= $data["id"] ?>" method="post">
                        <label>Dame el Número del aula:
                            <input type="number" name="numAula" id="numAula" value="<?= $data["numAula"] ?>">
                        </label>
                        <br>
                        <label>Selecciona un grupo: <br>
                            <?php
                            
                            foreach ($data["allGrupos"] as $grupo) {
                                echo "<input type=\"checkbox\" name=\"grupo[]\" value=\"" . $grupo['nombre'] . "\"";
                                if ($data["grupo1"] == $grupo['nombre'] || $data["grupo2"] == $grupo['nombre']) {
                                    echo " checked";
                                }
                        ...
                        <label>Dame el Número de mesas:
                            <input type="number" name="numMesas" id="numMesas" value="<?= $data["numMesas"] ?>">
                        </label>
                        <br>
                        <input type="submit" name="editSubmit" value="Editar">
                        <a href="/admin/aulas/" style="background-color: #de1d1d;color: #fff;padding: 10px 20px;border: none;border-radius: 5px;cursor: pointer;font-size: 16px;">Cancelar</a>
                    </form>
                </div>
