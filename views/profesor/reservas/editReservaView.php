...
<head>
    ...
    <title>Editar Reserva-Centro Educativo IES Gran Capitán</title>
</head>
...
<body>
    ...
    <div class="divMainSection">
        ...
        ?>
        <form action="/admin/reservas/edit/<?= $data["id"] ?>" method="post" style="margin-bottom: 20px;">
            ...
            <select name="id_equipo" id="id_equipo" required>
                ...
                foreach ($equipos as $equipo) {
                    ...
                    if ($data["equipoActual"] === $equipo["id"]) {
                        echo " selected";
                    }
                    echo ">" . $equipo["codigo"] . "</option>";
                }
                ?>
            ...
            <select name="id_profesor" id="id_profesor" required>
                ...
                foreach ($profesores as $profesor) {
                    ...
                    if ($data["profesorActual"] === $profesor["id"]) {
                        echo " selected";
                    }
                    echo ">" . $profesor["nombre"] . "</option>";
                }
                ?>
            </select>
            <br>
            <label for="observaciones">Observaciones:</label>
            <textarea name="observaciones" id="observaciones"><?=$data["observaciones"]?></textarea>
            <br>
            <label for="fecha_inicio">Fecha de inicio (mínimo: día siguiente al actual):</label>
            <input type="date" name="fecha_inicio" id="fecha_inicio" value="<?php echo htmlspecialchars($data["fechaInicio"]); ?>" min="<?php echo date('Y-m-d', strtotime('+1 day')); ?>" required>
            <br><br>
            <label for="fecha_fin">Fecha de fin:</label>
            <input type="date" name="fecha_fin" id="fecha_fin" value="<?php echo htmlspecialchars($data["fechaFin"]); ?>" min="<?php echo date('Y-m-d', strtotime('+1 day')); ?>" required>
            <br><br>
